...
def register(request):
    if request.method == 'POST':  # user click SignUp button
        ...
        if form.is_valid():  # all information is valid
            ...
            user.address.address = form.cleaned_data.get('address')
            user.zipcode.zipcode = form.cleaned_data.get('zipcode')
            user.phone.phone = form.cleaned_data.get('phone')
            user.git.git = form.cleaned_data.get('git')
            user.date.date = form.cleaned_data.get('date')
